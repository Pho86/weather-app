   ...
   return (
      <>
         <motion.div className='bg-gradient-to-t from-neutral-100 to-zinc-50 border-2 rounded-lg px-10 py-6 flex flex-col gap-2 drop-shadow-md transition duration-100 hover:drop-shadow-xl cursor-pointer mt-16'
            ...
            drag={true}
            dragConstraints={{ left: -250, right: 250, top: -20, bottom: 200 }}
            dragElastic={false}
            dragTransition={{ bounceStiffness: 300, bounceDamping: 50 }}
         >

            <div className='flex justify-between gap-3'>
               <motion.h1 initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: .25, delay: .3 }} className='font-bold text-5xl'>{data.name.length > 15 ? data.name.substring(0, 20) + "..." : data.name} </motion.h1>
               <h2 className=''>{data.sys.country}</h2>
            </div>

            <div className='flex justify-between gap-6'>
               <div className='flex flex-col gap-2 justify-between'>
                  <motion.div initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: .25, delay: .45 }}>
                     <p>Temperature</p><p className='text-2xl'><span className='font-bold text-4xl'>{data.main.temp}</span>℃</p>
                  </motion.div>
                  <motion.div initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: .25, delay: .6 }}>
                     <p>Feels like</p><p className='text-2xl'><span className='font-bold text-4xl'>{data.main.feels_like}</span>℃</p>
                  </motion.div>
                  <motion.div initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: .25, delay: .75 }}>
               ...
               <div className=''>
...

                  {
                     data.weather && data.weather.map((w, i) => {
                        return <motion.div key={i} initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: .25, delay: .9 }}>
